...
def validate_project_input(project: ProjectCreate) -> None:
    ...
    MAX_BUDGET = 500_000
    if project.budget_min is not None and project.budget_min < 1:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Budget minimum must be at least $1",
        )
    if project.budget_max is not None and project.budget_max > MAX_BUDGET:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Budget cannot exceed ${MAX_BUDGET:,}",
        )
...
def list_projects(
    ...
    sort: Optional[str] = Query(
        "newest",
        description="Sort order: newest, oldest, budget_high, budget_low, most_proposals",
    ),
    experience_level: Optional[str] = Query(
        None, description="Filter by experience level: entry, intermediate, expert"
    ),
    budget_min: Optional[float] = Query(
        None, ge=0, description="Minimum budget filter"
    ),
    budget_max: Optional[float] = Query(
        None, ge=0, description="Maximum budget filter"
    ),
) -> dict:
    ...
    try:
        ...
        # Build WHERE clause — reused for both COUNT and main SELECT
        where_sql = " WHERE 1=1"
        where_params: list = []
        ...
        if project_status:
            ...
            where_params.append(project_status.lower())
        else:
            where_sql += " AND p.status = 'open'"

        if category:
            where_sql += " AND p.category = ?"
        ...
        if experience_level:
            ...
            where_sql += " AND p.experience_level = ?"
            where_params.append(experience_level.lower())

        if budget_min is not None:
            where_sql += " AND p.budget_max >= ?"
            where_params.append(budget_min)

        if budget_max is not None:
            where_sql += " AND p.budget_min <= ?"
            where_params.append(budget_max)

        if search:
            safe_search = sanitize_search_term(search)
            if safe_search:
                where_sql += " AND (p.title LIKE ? ESCAPE '\\' OR p.description LIKE ? ESCAPE '\\')"
                where_params.extend([f"%{safe_search}%", f"%{safe_search}%"])

        # COUNT total matching rows (no JOIN needed — filters only touch projects table)
        count_row = turso.fetch_one(
            f"SELECT COUNT(*) FROM projects p{where_sql}", where_params
        )
        total = int(count_row[0]) if count_row and count_row[0] is not None else 0

        # Main SELECT with proposal count via subquery
        ...
        sql += " LIMIT ? OFFSET ?"
        main_params = where_params + [limit, offset]

        result = turso.execute(sql, main_params)
        columns = result.get("columns", [])
        projects = []
        for row in result.get("rows", []):
            proj = _row_to_project(
                row, columns[:14]
            )  # First 14 columns are project fields
            proj["proposal_count"] = (
...
def get_projects_batch(ids: List[int]) -> List[dict]:
    """Get multiple projects by IDs - used for proposals page efficient loading"""
    if not ids or len(ids) > 50:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Provide 1-50 project IDs"
        )
...
def create_project(
    ...
    for field_name, value in [
        ...
        if not ok:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Project {field_name} rejected: {reason}",
            )
