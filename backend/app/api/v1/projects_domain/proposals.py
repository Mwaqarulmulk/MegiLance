...
from app.core.responses import validate_length, validate_range
...
from app.services.db_utils import paginate_params, sanitize_text
...
def _validate_proposal_input(proposal) -> None:
    ...
    if proposal.availability:
        validate_length(
            proposal.availability, "Availability", max_length=MAX_AVAILABILITY_LENGTH
        )

    # Attachments validation
    if proposal.attachments:
        validate_length(
            proposal.attachments, "Attachments", max_length=MAX_ATTACHMENTS_LENGTH
        )
    ...
    # Validate bid amount against the project's budget range
    if (
        proposal.bid_amount is not None
        and hasattr(proposal, "project_id")
        and proposal.project_id
    ):
        turso = get_turso_http()
        project_row = turso.fetch_one(
            "SELECT budget_min, budget_max, budget_type FROM projects WHERE id = ?",
            [proposal.project_id],
        )
        if project_row:
            budget_min = float(project_row[0] or 0)
            budget_max = float(project_row[1] or 999_999)
            bid = proposal.bid_amount
            if bid < max(1.0, budget_min * 0.5):  # allow up to 50% below minimum
                raise HTTPException(
                    status_code=400,
                    detail=f"Bid too low. Project budget starts at ${budget_min:.0f}",
                )
            if bid > budget_max * 2:  # allow up to 2x above maximum
                raise HTTPException(
                    status_code=400,
                    detail=f"Bid too high. Project budget is up to ${budget_max:.0f}",
                )
...
def create_draft_proposal(
    proposal: ProposalCreate, current_user: User = Depends(get_current_active_user)
):
    ...
    result = proposals_service.create_draft_proposal(
        current_user.id,
        {
            "project_id": proposal.project_id,
            "cover_letter": sanitize_text(proposal.cover_letter)
            if proposal.cover_letter
            else "",
            "bid_amount": draft_bid or 0,
            "estimated_hours": proposal.estimated_hours or 0,
            "hourly_rate": proposal.hourly_rate or 0,
            "availability": sanitize_text(proposal.availability)
            if proposal.availability
            else "",
...
def list_proposals(
    ...
    """List proposals for current user with pagination metadata"""
    offset, limit = paginate_params(page, page_size)
    user_type = _safe_str(current_user.user_type)
    user_type_lower = (user_type or "").lower()

    # COUNT total matching proposals (mirrors the WHERE logic in proposals_service.list_proposals)
    turso = get_turso_http()
    if user_type_lower == "freelancer":
        count_where = "WHERE freelancer_id = ?"
    ...
    if project_id:
        count_where += " AND project_id = ?"
        count_params.append(project_id)
    if status:
        count_where += " AND status = ?"
        count_params.append(status)
    count_row = turso.fetch_one(
        f"SELECT COUNT(*) FROM proposals {count_where}", count_params
    )
    ...
    if include and "project" in include.lower():
        enrich_ids = list(
            set(p.get("project_id") for p in proposals if p.get("project_id"))
        )

        if enrich_ids:
            placeholders = ",".join("?" for _ in enrich_ids)
            project_rows = turso.fetch_all(
                f"SELECT id, title, client_id FROM projects WHERE id IN ({placeholders})",
                enrich_ids,
            )
            # fetch_all returns list-of-lists: [id, title, client_id]
            project_map = {row[0]: row for row in project_rows}
...
def get_proposal(
    proposal_id: int, current_user: User = Depends(get_current_active_user)
):
...
def create_proposal(
    ...
    if not ok:
        raise HTTPException(
            ...
            detail=f"Cover letter rejected: {reason}",
        )
    ...
    # Check if already submitted — use Turso HTTP directly for a clean 409 conflict
    _turso = get_turso_http()
    _dup = _turso.fetch_one(
    ...
    client_id = proposals_service.get_project_client_id(proposal.project_id)
    if client_id:
        freelancer_name = getattr(current_user, "full_name", None) or getattr(
            current_user, "username", "A freelancer"
        )

        # Get client email
        turso = get_turso_http()
        client_row = turso.fetch_one("SELECT email, name FROM users WHERE id = ?", [client_id])
        client_email = client_row[1] if client_row and len(client_row) > 1 and client_row[1] else None
        client_name = client_row[2] if client_row and len(client_row) > 2 and client_row[2] else "Client"

        # Get project title
        project_row = turso.fetch_one("SELECT title FROM projects WHERE id = ?", [proposal.project_id])
        project_title = project_row[0] if project_row and project_row[0] else "your project"
        ...
        if client_email:
            ...
            background_tasks.add_task(
                email_service.send_proposal_received_notification,
                to_email=client_email,
                client_name=client_name,
                freelancer_name=freelancer_name,
                project_title=project_title,
                proposal_id=result.get("id") if isinstance(result, dict) else 0,
            )
...
def update_proposal(
    ...
    if not result:
        raise HTTPException(
            status_code=500, detail="Failed to retrieve updated proposal"
        )
    return result
...
def accept_proposal(
    ...
    if freelancer_id:
        ...
        turso = get_turso_http()
        freelancer_row = turso.fetch_one("SELECT email, name FROM users WHERE id = ?", [freelancer_id])
        freelancer_email = freelancer_row[1] if freelancer_row and freelancer_row[1] else None
        freelancer_name = freelancer_row[2] if freelancer_row and len(freelancer_row) > 2 and freelancer_row[2] else "Freelancer"
        ...
        if freelancer_email:
            from app.services.email_service import email_service
            background_tasks.add_task(
                email_service.send_proposal_accepted_notification,
...
def reject_proposal(
    ...
    result = proposals_service.reject_proposal(
        proposal_id, reason=sanitize_text(reason) if reason else None
    )
    if not result:
        raise HTTPException(
            status_code=500, detail="Failed to retrieve updated proposal"
    ...
    if freelancer_id:
        ...
        turso = get_turso_http()
        freelancer_row = turso.fetch_one("SELECT email FROM users WHERE id = ?", [freelancer_id])
        freelancer_email = freelancer_row[0] if freelancer_row and freelancer_row[0] else None
        ...
        if freelancer_email:
            ...
            background_tasks.add_task(
                ...
                to_email=freelancer_email,
                subject="Proposal Update - MegiLance",
                html_content=f"<html><body><h2>Proposal Update</h2><p>{msg}</p><p>Thank you for your interest. We encourage you to apply for other projects that match your skills.</p></body></html>",
                text_content=msg,
            )
...
def create_counter_offer(
    ...
    counter_data = {
        ...
        "counter_at": __import__("datetime")
...
@router.get("/stats/my")
def get_my_proposal_stats(current_user: User = Depends(get_current_active_user)):
    """
