...
def create_stripe_customer(
    ...
    try:
        ...
        customer = stripe_service.create_customer(
            email=payload.email, name=payload.name, metadata=metadata
        )
        ...
        raise HTTPException(
            ...
            detail=f"Payment failed: {type(e).__name__}",
        )
...
@router.get(
    "/payment-intents/{payment_intent_id}", response_model=PaymentIntentResponse
)
@api_rate_limit()
...
@router.post(
    "/payment-intents/{payment_intent_id}/confirm", response_model=PaymentIntentResponse
)
@api_rate_limit()
def confirm_payment_intent(
    ...
    try:
        payment_intent = stripe_service.confirm_payment_intent(
            payment_intent_id, payment_method=payload.payment_method
        )
...
@router.post(
    "/payment-intents/{payment_intent_id}/capture", response_model=PaymentIntentResponse
)
@api_rate_limit()
...
@router.post(
    "/payment-intents/{payment_intent_id}/cancel", response_model=PaymentIntentResponse
)
@api_rate_limit()
...
@router.post(
    "/refunds", response_model=RefundResponse, status_code=status.HTTP_201_CREATED
)
@api_rate_limit()
def create_refund(
    request: Request,
...
def attach_payment_method(
    ...
    try:
        payment_method = stripe_service.attach_payment_method(
            payload.payment_method_id, customer_id
        )
...
async def stripe_webhook(
    ...
    if not webhook_secret:
        logger.critical(
            "STRIPE_WEBHOOK_SECRET is not configured - webhooks will not process"
        )
        raise HTTPException(
    ...
    try:
        ...
        event = stripe_service.construct_webhook_event(
            payload.decode(), stripe_signature, webhook_secret
        )
        ...
        if event_type == "payment_intent.succeeded":
            ...
            if metadata.get("payment_id"):
                ...
                logger.info(
                    f"[STRIPE WEBHOOK] Payment {payment_id} marked as completed"
                )
            ...
            # Fund escrow if contract_id is in metadata
            if metadata.get("contract_id"):
                from app.db.turso_http import execute_query
                from datetime import datetime, timezone
                contract_id = int(metadata["contract_id"])
                now = datetime.now(timezone.utc).isoformat()
                try:
                    execute_query(
                        "UPDATE escrow SET status = 'funded', updated_at = ? WHERE contract_id = ? AND status = 'pending'",
                        [now, contract_id]
                    )
                    logger.info(f"[STRIPE WEBHOOK] Escrow funded for contract {contract_id}")
                except Exception as e:
                    logger.error(f"[STRIPE WEBHOOK] Failed to fund escrow for contract {contract_id}: {e}")

        elif event_type == "checkout.session.completed":
            session_id = event_data.get("id")
            amount = event_data.get("amount_total", 0) / 100.0
            metadata = event_data.get("metadata", {})
            ...
            if metadata.get("contract_id"):
                ...
                try:
                    ...
                    logger.info(f"[STRIPE WEBHOOK] Checkout session completed — escrow funded for contract {contract_id}")
                except Exception as e:
                    logger.error(f"[STRIPE WEBHOOK] Failed to fund escrow from checkout session: {e}")
            ...
            if metadata.get("payment_id"):
                ...
                logger.warning(
                    f"[STRIPE WEBHOOK] Payment {payment_id} marked as failed"
                )
