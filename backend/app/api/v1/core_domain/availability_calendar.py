...
async def get_calendar_sync_status(
    ...
    try:
        ...
        res = execute_query("SELECT provider, last_synced_at FROM calendar_sync WHERE user_id = ?", [user_id])
        for row in parse_rows(res):
            provider = row.get("provider")
            key = f"{provider}_calendar"
            if key in status:
                status[key]["connected"] = True
                status[key]["last_sync"] = row.get("last_synced_at")
    except Exception:
        pass
