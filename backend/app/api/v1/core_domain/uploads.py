...
# python-magic removed — magic-byte detection is done inline in validate_file_content()
...
# Magic-byte signatures used to verify actual file content
...
# Binary/text signatures that should NEVER be accepted regardless of MIME header
DANGEROUS_SIGS: list[bytes] = [
    b"<?php",
    b"<script",
    b"<html",
    b"#!/",
    b"MZ\x90\x00",  # Windows PE (EXE/DLL)
]
...
def sanitize_filename(filename: str) -> str:
    ...
    return f"{name}_{uuid.uuid4().hex[:8]}{ext}"[:255]
...
def validate_file_content(file_content: bytes, allowed_types: set[str]) -> None:
    """Validate file content using magic bytes. Raises HTTPException if invalid."""
    if not file_content or len(file_content) < 4:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Empty or too-small file",
        )

    # Check dangerous signatures first — use ASCII-safe case-insensitive comparison
    header = file_content[:12]
    header_lower = header.decode("ascii", errors="replace").lower()
    for sig in DANGEROUS_SIGS:
        sig_lower = sig.decode("ascii", errors="replace").lower()
        if header_lower[: len(sig_lower)] == sig_lower:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    for sig, mime in MAGIC_SIGNATURES.items():
        if file_content[: len(sig)] == sig:
            detected = mime
            break
    # WebP: RIFF....WEBP (needs check at offset 8, not covered by simple prefix)
    if detected is None and len(file_content) >= 12:
        if file_content[:4] == b"RIFF" and file_content[8:12] == b"WEBP":
            detected = "image/webp"

    # If we positively identified the type and it is not in the allowed set, reject
    if detected and detected not in allowed_types:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Invalid file type. Detected: {detected}",
        )


def validate_file(file: UploadFile, allowed_types: set[str], max_size: int) -> bytes:
    ...
    # Validate actual content via magic bytes — raises HTTPException on mismatch
...
def delete_uploaded_file(file_url: str):
    ...
    if (
        ...
        and getattr(settings, "aws_bucket_name", None)
    ):
        try:
            s3 = get_s3_client()
...
async def upload_avatar(
    ...
    file_url = save_uploaded_file(
        file_content, file.filename or "avatar.jpg", "avatars", file.content_type
    )

    # Update user profile
    update_user_avatar(current_user["id"], file_url)

    return {"url": file_url, "message": "Avatar uploaded successfully"}
...
async def upload_portfolio_image(
    ...
    file_url = save_uploaded_file(
        file_content, file.filename or "portfolio.jpg", "portfolio", file.content_type
    )

    return {"url": file_url, "message": "Portfolio image uploaded successfully"}
...
async def upload_document(
    ...
    # Save document
    file_url = save_uploaded_file(
        file_content, file.filename or "document.pdf", "documents", file.content_type
    )
...
async def delete_uploaded_file(
    request: Request, file_path: str, current_user=Depends(get_current_user)
):
    """
