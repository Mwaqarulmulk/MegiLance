...
async def create_review(
    review_data: ReviewCreateRequest, current_user=Depends(get_current_user)
):
    """
    Create a new review.
    ...
    if contract.get("status") != "completed":
        raise HTTPException(
            status_code=400, detail="Can only review completed contracts"
        )

    # Verify user is part of the contract
    if user_id not in [contract.get("client_id"), contract.get("freelancer_id")]:
        raise HTTPException(
            status_code=403, detail="Only contract parties can create reviews"
        )
    ...
    other_party_id = (
        contract.get("freelancer_id")
        if user_id == contract.get("client_id")
        else contract.get("client_id")
    )
    if reviewed_user_id != other_party_id:
        raise HTTPException(
            status_code=400, detail="Can only review the other party in the contract"
        )

    # --- FIX 5a: Verify a completed contract exists between reviewer and reviewee ---
    _turso = get_turso_http()
    _completed_contract = _turso.fetch_one(
        """SELECT id FROM contracts
           WHERE ((client_id = ? AND freelancer_id = ?) OR (client_id = ? AND freelancer_id = ?))
             AND status = 'completed'
           LIMIT 1""",
        [user_id, reviewed_user_id, reviewed_user_id, user_id],
    )
    if not _completed_contract:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You can only review users you have completed a contract with",
        )

    # --- FIX 5b: Prevent duplicate reviews between the same pair of users ---
    _existing_review = _turso.fetch_one(
    ...
    # Check for existing review (per-contract guard, kept for backward compat)
    ...
    # FIX P0-6: Map input field name (review_text) → DB column name (comment)
    review_comment = (
        review_data.review_text
        if hasattr(review_data, "review_text")
        else getattr(review_data, "comment", None)
    )
...
async def list_reviews(
    ...
    sort: Optional[str] = Query(
        "newest", description="Sort: newest, oldest, highest, lowest"
    ),
...
async def get_reviews_for_user(
    ...
    rows = reviews_service.query_reviews(
        where_sql, params, order_by="r.created_at DESC"
    )
    output = []
    for row in rows:
        breakdown = {}
...
async def get_reviews_for_contract(
    contract_id: int, current_user=Depends(get_current_user)
):
...
async def get_review(review_id: int, current_user=Depends(get_current_user)):
    ...
    if not row.get("is_public"):
        if role.lower() != "admin" and user_id not in [
            row.get("reviewer_id"),
            row.get("reviewee_id"),
        ]:
            raise HTTPException(
                status_code=403, detail="You don't have permission to view this review"
            )
...
async def update_review(
    ...
    if user_id != review.get("reviewer_id"):
        raise HTTPException(
            status_code=403, detail="You don't have permission to update this review"
        )
...
async def delete_review(review_id: int, current_user=Depends(get_current_user)):
    ...
    if role.lower() != "admin" and user_id != review.get("reviewer_id"):
        raise HTTPException(
            status_code=403, detail="Only the reviewer or admin can delete this review"
        )
...
async def respond_to_review(
    ...
    response_text: str = Query(
        ..., min_length=10, max_length=2000, description="Response text"
    ),
    current_user=Depends(get_current_user),
):
    """
    ...
    if user_id != review.get("reviewee_id"):
        raise HTTPException(
            status_code=403, detail="Only the reviewed user can respond to this review"
        )
    ...
    if breakdown.get("response"):
        raise HTTPException(
            status_code=400, detail="You have already responded to this review"
        )
...
async def get_review_sentiment(
    ...
    if sentiment is None:
        raise HTTPException(
            status_code=503, detail="Sentiment analysis service unavailable"
        )
    return sentiment
...
async def get_user_review_sentiment(
    ...
    review_dicts = [
        {"text": r.get("comment", ""), "rating": r.get("rating")}
        for r in rows
        if r.get("comment")
    ]
    if not review_dicts:
