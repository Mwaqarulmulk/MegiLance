...
from app.schemas.user import ProfileCompleteUpdate, UserCreate, UserRead, UserUpdate
    ...
    if user_dict.get("skills"):
        try:
            ...
            # If it's comma separated
                ...
                user_dict["skills"] = [
                    s.strip() for s in user_dict["skills"].split(",")
                ]
...
def change_password(
    ...
    from app.core.security import add_token_to_blacklist, verify_password
...
def complete_user_profile(
    profile_data: ProfileCompleteUpdate, current_user: User = Depends(get_current_user)
) -> dict:
    ...
    try:
        ...
        if profile_data.location:
            location = _validate_string(
                profile_data.location, "Location", MAX_LOCATION_LENGTH
            )
            if location:
        ...
        if profile_data.availability:
            ...
            if (
                profile_data.availability.lower().replace("-", "_")
        ...
        if profile_data.portfolioItems:
            ...
            for idx, item in enumerate(
                profile_data.portfolioItems[:10]
            ):  # Max 10 items
                validated_item = {
                    "title": _validate_string(
                        item.title, f"Portfolio item {idx + 1} title", 200
                    ),
                    "description": _validate_string(
                        item.description, f"Portfolio item {idx + 1} description", 1000
                    ),
                    "url": _validate_url(item.url, f"Portfolio item {idx + 1}"),
                    "imageUrl": _validate_url(
                        item.imageUrl, f"Portfolio item {idx + 1} image"
                    ),
        ...
        if profile_data.websiteUrl:
            extra_data["website_url"] = _validate_url(
                profile_data.websiteUrl, "Website"
            )
...
def get_notification_preferences(
    ...
    try:
        turso = get_turso_http()
        row = turso.fetch_one(
            "SELECT notification_preferences FROM users WHERE id = ?", [current_user.id]
        )
