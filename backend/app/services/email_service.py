# @AI-HINT: Email sending service using Resend API (primary) with SMTP fallback
# Email Service Configuration
# This module provides email sending capabilities using Resend API and SMTP
...
class EmailService:
    ...
    Primary: Resend API (https://resend.com)
    Fallback: SMTP
    ...
    def _send_via_resend(
        ...
        try:
            ...
            if response.status_code in (200, 201):
                ...
                # Fallback to SMTP on Resend failure
    ...
    # ── Async wrapper methods (non-blocking via thread pool) ──────────

    async def send_email_async(self, *args, **kwargs) -> bool:
        """Non-blocking email using thread pool executor."""
        import asyncio
    ...
    async def send_verification_email_async(
        ...
        return await asyncio.get_event_loop().run_in_executor(
            ...
            lambda: self.send_verification_email(
                to_email, user_name, verification_token
            ),
        )
