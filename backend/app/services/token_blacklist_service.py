...
def add_token_to_blacklist(
    token: str, expires_at: datetime, reason: str = "logout"
) -> None:
    """
...
def is_token_blacklisted(token: str) -> bool:
    ...
    try:
        result = execute_query(
            "SELECT expires_at FROM revoked_tokens WHERE token_hash = ?", [token_hash]
        )
...
def cleanup_expired_tokens() -> int:
    ...
    try:
        ...
        # Count first
        result = execute_query(
            "SELECT COUNT(*) as cnt FROM revoked_tokens WHERE expires_at < ?", [now_str]
        )
        rows = parse_rows(result)
        count = int(rows[0].get("cnt", 0)) if rows else 0

        if count > 0:
            execute_query("DELETE FROM revoked_tokens WHERE expires_at < ?", [now_str])
...
def _cleanup_single(token_hash: str) -> None:
    ...
    try:
        execute_query("DELETE FROM revoked_tokens WHERE token_hash = ?", [token_hash])
    except Exception:
        pass
...
def _trim_cache() -> None:
    ...
    if len(_clean_cache) > _CACHE_MAX_SIZE:
        # Remove oldest entries (by time_checked timestamp, smallest first)
        sorted_keys = sorted(_clean_cache.keys(), key=lambda k: _clean_cache[k])
        for k in sorted_keys[: len(_clean_cache) - _CACHE_MAX_SIZE]:
            del _clean_cache[k]
