...
# In-memory dict tracking recent failed login attempts for account lockout
_failed_login_attempts: dict[
    str, list[float]
] = {}  # email_lower -> [attempt_timestamps]
...
def authenticate_user(email: str, password: str) -> Optional[Any]:
    ...
    try:
        ...
        # --- Account lockout: rate-limit failed login attempts (15-min sliding window) ---
        _now = time.time()
        _attempts = _failed_login_attempts.get(email_lower, [])
        # Prune attempts older than 15 minutes (900 seconds)
        _attempts = [ts for ts in _attempts if _now - ts < 900]
        _failed_login_attempts[email_lower] = _attempts
        if len(_attempts) >= 5:
            raise HTTPException(
        ...
        if not hashed_pw or not verify_password(password, hashed_pw):
            ...
            _failed_login_attempts[email_lower].append(time.time())
            return None
        ...
        _failed_login_attempts.pop(email_lower, None)  # Clear on successful login
...
def create_access_token(
    ...
    return _create_token(data, timedelta(minutes=expires_delta_minutes), "access")
...
def get_current_active_user(
    current_user: Union[User, UserProxy] = Depends(get_current_user),
) -> Union[User, UserProxy]:
...
def require_admin(
    current_user: Union[User, UserProxy] = Depends(get_current_active_user),
) -> Union[User, UserProxy]:
