...
const Settings: React.FC = () => {
  ...
  const portalArea =
    typeof window !== "undefined"
      ? localStorage.getItem("portal_area") || "client"
      : "client";
  const basePath =
    portalArea === "admin"
      ? "/admin"
      : portalArea === "freelancer"
        ? "/freelancer"
        : "/client";
        ...
        items: [
          {
            icon: <User size={20} />,
            title: "Profile",
            desc: "Personal information, bio, and avatar",
        ...
        items: [
          {
            ...
            title: "Appearance",
            desc: "Theme, language, and display options",
            path: `${basePath}/settings`,
          },
          {
            icon: <Globe size={20} />,
            title: "Language & Region",
            desc: "Locale, timezone, and date format",
            path: `${basePath}/settings?tab=locale`,
          },
        ],
      },
      {
      ...
      <div className={commonStyles.categories}>
        {filteredCategories.map((category) => (
          <section key={category.label} className={commonStyles.category}>
            <h2
              className={cn(
                commonStyles.categoryLabel,
                themeStyles.categoryLabel,
              )}
            >
              {category.label}
            </h2>
            <div className={commonStyles.categoryGrid}>
              {category.items.map((item) => (
                ...
                >
                  <div
                    className={cn(commonStyles.cardIcon, themeStyles.cardIcon)}
                  >
                    {item.icon}
                  </div>
                  <div className={commonStyles.cardContent}>
                    <div className={commonStyles.cardTitleRow}>
                      ...
                      {item.badge && (
                        <span
                          className={cn(
                            commonStyles.cardBadge,
                            themeStyles.cardBadge,
                          )}
                        >
                          {item.badge}
                        </span>
                    ...
                    <p
                      className={cn(
                        commonStyles.cardDesc,
                        themeStyles.cardDesc,
                      )}
                    >
                      {item.desc}
                    </p>
                  ...
                  <ChevronRight
                    size={16}
                    className={cn(
                      commonStyles.cardArrow,
                      themeStyles.cardArrow,
                    )}
                  />
                </button>
              ))}
