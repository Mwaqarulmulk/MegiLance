...
import {
  ...
  TrendingUp, Shield, BarChart2, Clock, Sparkles,
} from 'lucide-react';
  ...
  return (
    <div className={cs.resultsContainer}>
      ...
      <div className={cn(cs.priceHero, ts.priceHero, 'relative overflow-hidden')}>
        ...
        <div className={cn(cs.priceHeroValue, ts.priceHeroValue)}><NumberTicker value={result.budget.total} prefix={`${cur} `} /></div>
...
function buildScopeReport(r: PlanResult): AIReport {
  const cur = r.budget.currency;
  const totalHours = r.budget.team_breakdown.reduce((sum, item) => sum + item.total_hours, 0);
  return {
    toolName: 'AI Scope & Project Planner',
    title: 'Project Scope & Budget Report',
    subtitle: `Project Title: ${r.project.name || 'New Project'} · Complexity: ${r.project.complexity_label}`,
    fileBaseName: 'megilance-scope-report',
    kpis: [
      { label: 'Total Estimated Cost', value: `${cur} ${r.budget.total.toLocaleString()}`, hint: `Risk Buffer: ${cur} ${r.budget.risk_buffer.toLocaleString()}` },
      { label: 'Timeline Duration', value: `${r.timeline.total_weeks} weeks`, hint: `${r.timeline.total_months} months` },
      { label: 'Team Size', value: `${r.resources.team_size} members`, hint: `${totalHours} total hours` },
      { label: 'Completeness Score', value: `${r.completeness.score}/100`, hint: r.completeness.level },
    ],
    meta: [
      { label: 'Category', value: r.project.category_label },
      { label: 'Complexity', value: r.project.complexity_label },
    ],
    sections: [
      {
        heading: 'Project Phases & Timeline',
        blocks: [
          {
            kind: 'table',
            columns: ['Phase', 'Weeks', 'Start-End Week', 'Description', 'Budget Allocation'],
            rows: r.timeline.phases.map(p => [
              p.name,
              p.weeks,
              `Wk ${p.start_week} - Wk ${p.end_week}`,
              p.description,
              `${p.percent_of_total}%`,
            ]),
          }
      ...
      {
        heading: 'Team Composition & Cost Breakdown',
        blocks: [
          {
            kind: 'table',
            columns: ['Role', 'Hourly Rate', 'Total Hours', 'Total Cost'],
            rows: r.budget.team_breakdown.map(tb => [
              tb.role,
              `${cur} ${tb.rate}/hr`,
              tb.total_hours,
              `${cur} ${tb.cost.toLocaleString()}`,
            ]),
          }
        ]
      },
        ...
        blocks: [
          {
            kind: 'keyValue',
            rows: r.risks.map(ri => ({
              label: `${ri.title} (${ri.severity})`,
              value: `Mitigation: ${ri.mitigation}`
            }))
          }
        ]
      },
      {
        heading: 'Project Scope Recommendations',
        blocks: [
          {
            kind: 'bullets',
            items: r.recommendations.map(re => `${re.title}: ${re.message}`)
          }
        ]
      }
    ]
...
const getScopeSummaryText = (result: PlanResult) => {
  const cur = result.budget.currency;
  const totalHours = result.budget.team_breakdown.reduce((sum, item) => sum + item.total_hours, 0);
  return `MEGILANCE PROJECT SCOPE & BUDGET REPORT
----------------------------------------
Project: ${result.project.name || 'New Project'}
Category: ${result.project.category_label}
Complexity: ${result.project.complexity_label}

BUDGET SUMMARY:
- Total Budget: ${cur} ${result.budget.total.toLocaleString()}
- Base Labor Cost: ${cur} ${result.budget.labor_cost.toLocaleString()}
- Risk Buffer: ${cur} ${result.budget.risk_buffer.toLocaleString()} (${result.budget.risk_buffer_percent}%)
- Monthly Burn Rate: ${cur} ${result.budget.monthly_burn_rate.toLocaleString()}/mo

TIMELINE SUMMARY:
- Duration: ${result.timeline.total_weeks} weeks (${result.timeline.total_months} months)
- Start Date: ${result.timeline.start_date}
- Total Work Hours: ${totalHours} hours

TEAM DETAILS:
- Total Team Size: ${result.resources.team_size} members
${result.budget.team_breakdown.map(tb => `  * ${tb.role} (${tb.total_hours} hrs @ ${cur}${tb.rate}/hr): ${cur}${tb.cost.toLocaleString()}`).join('\n')}

PHASES:
${result.timeline.phases.map(p => `- Phase ${p.number}: ${p.name} (${p.weeks} wks, Wk ${p.start_week}-${p.end_week}): ${p.percent_of_total}% of budget\n  Description: ${p.description}`).join('\n')}

RISKS:
${result.risks.map(ri => `- [${ri.severity.toUpperCase()}] ${ri.title}: ${ri.message}\n  Mitigation: ${ri.mitigation}`).join('\n')}
    ...
    <div className={cn(cs.container, ts.container, 'relative overflow-hidden')}>
      ...
      <header className={cn(cs.header, 'relative z-10')}>
        <span className="mb-3 inline-flex justify-center">
          <ShineBadge className={cn(isDark ? 'text-blue-100' : 'text-blue-700')}><Layers size={14} /> AI-Powered</ShineBadge>
        </span>
        <h1 className={cn(cs.title, ts.title)}><AnimatedGradientText>Scope & Budget Planner</AnimatedGradientText></h1>
          ...
          {step === 3 && result && (
            <div className={cn(cs.actionsBar, "flex flex-wrap gap-2 items-center")}>
              ...
              <button 
                ...
                onClick={() => {
                  navigator.clipboard.writeText(getScopeSummaryText(result));
                  alert("Summary copied to clipboard!");
                }}
              >
                Copy Summary
              </button>
              <ExportMenu report={() => buildScopeReport(result)} />
            </div>
          )}
        </div>
      )}
