            ...
            {analysisResult && !isLoading && (
              ...
              >
                ...
                <div className={cn(commonStyles.resultActions, themeStyles.resultActions, "flex flex-wrap gap-2 items-center")}>
                  ...
                  <ExportMenu report={() => buildFraudReport(analysisResult)} />
                </div>
              </motion.div>
            )}
          </AnimatePresence>
...
function buildFraudReport(r: AnalysisResult): AIReport {
  return {
    toolName: 'AI Fraud & Scam Checker',
    title: 'AI Security & Risk Assessment Report',
    subtitle: `Confidence Score: ${r.confidence}% · Risk Level: ${r.riskLevel}`,
    fileBaseName: 'megilance-fraud-check',
    kpis: [
      { label: 'Security Score', value: `${r.score}/100`, hint: `${r.riskLevel} Risk` },
      { label: 'Confidence Score', value: `${r.confidence}%` },
      { label: 'Issues Found', value: `${r.warnings.length} detected` },
    ],
    sections: [
      {
        heading: 'Security Flag Details',
        blocks: [
          {
            kind: 'bullets',
            items: r.warnings.length > 0 ? r.warnings : ['No security flags or patterns detected. The job details look clean and standard.'],
          }
        ]
      },
      {
        heading: 'Detailed Warnings & Recommendations',
        blocks: [
          {
            kind: 'keyValue',
            rows: r.details ? r.details.map(d => ({
              label: `${d.category} (${d.severity})`,
              value: d.detail
            })) : [
              { label: 'Status', value: 'Low Risk Profile' },
              { label: 'Recommendation', value: 'Always execute work through MegiLance escrow to guarantee secure payments.' }
            ]
          }
        ]
      }
...
const getFraudSummaryText = (result: AnalysisResult) => {
  return `MEGILANCE SECURITY & FRAUD ANALYSIS REPORT
------------------------------------------
...
ISSUES DETECTED:
${result.warnings.length > 0 ? result.warnings.map(w => `- ${w}`).join('\n') : 'No security flags or fraud patterns detected.'}

RECOMMENDATION:
Always write contract agreements and process payouts through MegiLance's secure escrow system to prevent financial scams and disputes.
