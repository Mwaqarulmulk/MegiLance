...
import DataToolbar, {
  SortOption,
} from "@/app/components/organisms/DataToolbar/DataToolbar";
...
const allStatuses: Proposal["status"][] = [
  "Draft",
  "Submitted",
  "Interview",
  "Rejected",
...
const mapAPIStatus = (status: string, isDraft: boolean): Proposal["status"] => {
  ...
  if (normalizedStatus === "pending" || normalizedStatus === "submitted")
    return "Submitted";
  if (normalizedStatus === "interview" || normalizedStatus === "shortlisted")
    return "Interview";
...
const Proposals: React.FC = () => {
  ...
  const [sortKey, setSortKey] = usePersistedState<keyof Proposal>(
    "freelancer:proposals:sortKey",
    "dateSubmitted",
  );
  const [sortDir, setSortDir] = usePersistedState<"asc" | "desc">(
    "freelancer:proposals:sortDir",
    "desc",
  );
  ...
  const [pageSize, setPageSize] = usePersistedState<number>(
    "freelancer:proposals:pageSize",
    6,
  );
  const [statusFilters, setStatusFilters] = usePersistedState<
    Proposal["status"][]
  >("freelancer:proposals:statusFilters", []);
...
// Fetch proposals from API
  ...
  const filtered = useMemo(() => {
    ...
    const byQuery = (p: Proposal) =>
      !qLower ||
      p.jobTitle.toLowerCase().includes(qLower) ||
      p.clientName.toLowerCase().includes(qLower);
    const byStatus = (p: Proposal) =>
  ...
  const confirmWithdraw = async () => {
    if (pendingWithdrawId) {
      try {
        await proposalsApi.withdraw(pendingWithdrawId!);
        toaster.notify({
          title: "Success",
          description: "Proposal successfully withdrawn.",
          variant: "success",
        });
        fetchProposals();
      } catch (err) {
        toaster.notify({
          title: "Error",
          description:
            "Could not withdraw this proposal. It may have already been accepted or removed.",
          variant: "danger",
        });
      }
    ...
    <PageTransition>
      <div className={cn(commonStyles.container, styles.container)}>
        <ScrollReveal>
          <header className={cn(commonStyles.header, styles.header)}>
            ...
            <p className={cn(commonStyles.subtitle, styles.subtitle)}>
              Track your submitted proposals, monitor their status, and manage
              pending bids.
            </p>
          </header>
        </ScrollReveal>
        ...
        {error ? (
          ...
          <>
            ...
            {loading ? (
              ...
              <StaggerContainer
                delay={0.2}
                className={cn(commonStyles.grid, styles.grid)}
              >
                {paged.map((proposal) => (
                  <ProposalCard
                    key={proposal.id}
              ...
              <ScrollReveal delay={0.2}>
                <div className={cn(commonStyles.emptyState, styles.emptyState)}>
                  <h3
                    className={cn(commonStyles.emptyTitle, styles.emptyTitle)}
                  >
                    No Proposals Found
                  </h3>
        ...
        <Modal
          ...
          size="small"
        >
          <p className={cn(commonStyles.modalText, styles.modalText)}>
            Are you sure you want to withdraw this proposal? This action cannot
            be undone.
          </p>
