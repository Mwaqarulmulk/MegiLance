...
const NotificationSettingsPage = () => {
  ...
  const loadSettings = async () => {
    try {
      const response = (await usersApi.getNotificationPreferences()) as any;

      if (response) {
        setSettings({
          newJobAlerts:
            response.new_job_alerts ??
            response.newJobAlerts ??
            defaultSettings.newJobAlerts,
          proposalStatusUpdates:
            response.proposal_status_updates ??
            response.proposalStatusUpdates ??
            defaultSettings.proposalStatusUpdates,
          messageNotifications:
            response.message_notifications ??
            response.messageNotifications ??
            defaultSettings.messageNotifications,
          paymentConfirmations:
            response.payment_confirmations ??
            response.paymentConfirmations ??
            defaultSettings.paymentConfirmations,
          weeklySummary:
            response.weekly_summary ??
            response.weeklySummary ??
            defaultSettings.weeklySummary,
        });
      }
    } catch (error) {
      ...
      // Keep default settings on load failure — the user can still adjust and save
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await (usersApi.updateNotificationPreferences as any)({
  ...
  return (
    <div className={cn(commonStyles.formContainer, styles.formContainer)}>
      <header className={cn(commonStyles.formHeader, styles.formHeader)}>
        <h2 className={cn(commonStyles.formTitle, styles.formTitle)}>
          Notifications
        </h2>
      ...
      <form onSubmit={handleSubmit} className={commonStyles.form}>
        <div className={commonStyles.inputGroup}>
          ...
          <NotificationOption
            ...
            onChange={(value) =>
              handleSettingChange("proposalStatusUpdates", value)
            }
          />
          <NotificationOption
          ...
          <NotificationOption
            ...
            onChange={(value) =>
              handleSettingChange("paymentConfirmations", value)
            }
          />
          <NotificationOption
            id="weekly-summary"
