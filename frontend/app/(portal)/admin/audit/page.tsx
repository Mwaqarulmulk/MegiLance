...
export default function AuditTrailPage() {
  ...
  const handleExport = async () => {
    const CSV_HEADER = "Date,User,Action,Details,IP Address";
    ...
    try {
      // Use currently loaded logs, or fetch a fresh batch if the table is empty.
      let logsToExport: AuditLog[] = logs;

      if (logsToExport.length === 0) {
        try {
          const result = (await auditTrailApi.getEvents({
            ...filters,
            page: 1,
            page_size: 1000,
          } as any)) as any;
      ...
      // No logs — export a header-only template
      if (logsToExport.length === 0) {
        triggerDownload(CSV_HEADER + "\n");
        showToast("Exported header-only CSV (no logs matched filters)");
        return;
      }
      ...
      triggerDownload([CSV_HEADER, ...rows].join("\n"));
      showToast(`Exported ${logsToExport.length} audit log(s)`);
    } catch (err) {
      if (process.env.NODE_ENV === "development") {
    ...
    <PageTransition>
      <div className={cn(commonStyles.container, themeStyles.container)}>
        ...
        <ScrollReveal delay={0.2}>
          <div
            className={cn(commonStyles.filtersCard, themeStyles.filtersCard)}
          >
                        ...
                        <tr key={log.id} className={cn(themeStyles.logRow)}>
                          ...
                          <td>
                            <div className={commonStyles.userCell}>
                              ...
                              <span className={commonStyles.userRole}>
                                {log.user_role}
                              </span>
                            </div>
                          </td>
                          ...
                          <td>
                            <span
                              className={cn(
                                commonStyles.resourceBadge,
                                themeStyles.resourceBadge,
                              )}
                            >
                              {log.resource_type}
                            </span>
                            {log.resource_id && (
                              <span className={commonStyles.resourceId}>
                                #{log.resource_id.slice(-8)}
                              </span>
                            )}
                          </td>
                          <td className={commonStyles.ipCell}>
                            {log.ip_address}
                          </td>
                          <td>
                            <Button
              ...
              <div className={commonStyles.detailSection}>
                ...
                <div className={commonStyles.detailGrid}>
                  ...
                  <div className={commonStyles.fullWidth}>
                    ...
                    <span className={commonStyles.userAgent}>
              ...
              {selectedLog.details &&
                Object.keys(selectedLog.details).length > 0 && (
                  <div className={commonStyles.detailSection}>
                    ...
                    <pre
                      className={cn(
                        commonStyles.jsonBlock,
                        themeStyles.jsonBlock,
                      )}
                    >
