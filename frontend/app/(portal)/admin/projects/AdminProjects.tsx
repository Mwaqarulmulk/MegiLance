...
const statusDotClass = (status: ProjectRow["status"]) => {
  switch (status) {
    case "Planned":
      return common.badgeDotPlanned;
    case "In Progress":
      return common.badgeDotInProgress;
    case "Blocked":
      return common.badgeDotBlocked;
    case "Completed":
      return common.badgeDotCompleted;
  }
  return undefined;
};
...
const AdminProjects: React.FC = () => {
  ...
  const { projects, loading, error, refetch } = useAdminData();
  ...
  React.useEffect(() => {
    setPage(1);
  }, [sortKey, sortDir, query, status, pageSize]);
  ...
  const searchFreelancers = async () => {
    ...
    try {
      const response = await fetch(
        `/api/v1/admin/users?role=freelancer&search=${encodeURIComponent(freelancerSearch)}&page_size=10`,
        {
          headers: {
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      toaster?.notify?.({
        title: "Assigned",
        description: `Freelancer assigned to project #${assignProjectId}`,
        variant: "success",
      });
  ...
  const handleCreateProject = async () => {
    ...
    try {
      ...
      setCreateForm({ title: "", description: "", budget: "", category: "" });
  ...
  return (
    <PageTransition className={cn(common.page, themed.themeWrapper)}>
      <div className={common.container}>
        <ScrollReveal className={common.header}>
          <div>
            ...
            <p className={cn(common.subtitle, themed.subtitle)}>
              Platform-wide projects overview. Filter by status and search by
              name/client.
            </p>
          </div>
        ...
        <ScrollReveal
          className={common.tableWrap}
          aria-busy={loading || undefined}
          delay={0.2}
        >
          {error && (
            <div className={common.error}>Failed to load projects.</div>
          )}
          <div className={cn(common.toolbar)}>
            <div className={common.controls}>
              <label className={common.srOnly} htmlFor="sort-key">
                Sort by
              </label>
              <select
                id="sort-key"
              ...
              <label className={common.srOnly} htmlFor="sort-dir">
                Sort direction
              </label>
              <select
                id="sort-dir"
              ...
              <label className={common.srOnly} htmlFor="page-size">
                Rows per page
              </label>
              <select
                id="page-size"
          ...
          {sorted.length === 0 && !loading && (
            <div className={cn(common.empty)} role="status" aria-live="polite">
        ...
        >
          <div
            className={cn(
              common.container,
              themed.themeWrapper,
              "max-w-lg w-full mx-4 rounded-xl shadow-xl",
            )}
          ...
          >
            ...
            <div className="p-4 space-y-4">
              <div>
                ...
                <input
                  type="text"
                  className={cn(common.input, themed.input, "w-full")}
          ...
          >
            ...
            >
              <h2 className="text-lg font-semibold">
                Assign Freelancer to Project #{assignProjectId}
              </h2>
              <button
              ...
              {selectedFreelancer && (
                <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                  ...
                  <div className="font-medium">
                    {selectedFreelancer.name || selectedFreelancer.email}
                  </div>
                </div>
              )}
            </div>
