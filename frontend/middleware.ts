...
export function middleware(request: NextRequest) {
  ...
  const securityHeaders: Record<string, string> = {
    ...
    "Permissions-Policy":
      "camera=(), microphone=(), geolocation=(self), payment=(self), usb=(), bluetooth=(), accelerometer=(), gyroscope=()",
  ...
  // Server-side onboarding redirect: route /onboarding → /onboarding/client for client role
  if (pathname === "/onboarding" && isTokenValid && authToken) {
    try {
      const base64Url = authToken.split(".")[1];
      const base64 = base64Url.replace(/-/g, "+").replace(/_/g, "/");
      const payload = JSON.parse(atob(base64));
      ...
      /* fall through to default onboarding */
    }
  }
 ...
 * Following Next.js best practice: exclude static files and API routes
 */
export const config = {
  matcher: [
    /*
