    ...
    async function load() {
      ...
      try {
        ...
        const [
          ...
          fetchWithFallback<SystemStats | null>(
            api.admin.getDashboardStats(),
            null,
          ),
          fetchWithFallback<RecentActivity[]>(
        ...
        // Map users
        const mappedUsers: AdminUser[] = (usersJson.users || []).map(
          (u: AdminUserResponse) => ({
            id: String(u.id),
            name: u.name || "Unknown",
            email: u.email || "",
        ...
        // Map projects
        const mappedProjects: AdminProject[] = (projJson.projects || []).map(
        ...
        // Map payments
        const mappedPayments: AdminPayment[] = (payJson.payments || []).map(
          (p: AdminPaymentResponse) => ({
            id: String(p.id),
        ...
        // Map tickets
        const mappedTickets: AdminSupportTicket[] = (supJson.tickets || []).map(
          (t: AdminTicketResponse) => ({
            id: String(t.id),
        ...
        setAI(
          aiJson && Object.keys(aiJson).length > 0
            ? (aiJson as AdminAIMetric)
            : null,
        ); // Placeholder
          ...
          const kpiList: AdminKPI[] = [
            {
              id: "users",
              label: "Total Users",
              value: dashJson.total_users?.toLocaleString() ?? "0",
            },
            {
              id: "projects",
              label: "Active Projects",
              value: dashJson.active_projects?.toLocaleString() ?? "0",
            },
            {
              id: "revenue",
              label: "Revenue",
              value: `$${((dashJson.total_revenue ?? 0) / 1000).toFixed(0)}k`,
            },
            {
              id: "proposals",
              label: "Pending Proposals",
              value: dashJson.pending_proposals?.toLocaleString() ?? "0",
            },
          ];
          setKPIs(kpiList);
        } else {
